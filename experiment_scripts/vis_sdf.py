...
p.add_argument('--vis', default='y', help='Whether to visualize.')
p.add_argument('--slice_npz', default=None, type=str, help='Whether to load in pre-computed slice quantities.')
p.add_argument('--slice_var', default=0, type=float, help='Where to slice.')
...
if 'siren' in opt.checkpoint_path:
    opt.mode = 'mlp'
elif 'ingp' in opt.checkpoint_path:
    opt.mode = 'ingp'
...
if opt.vis == 'y':
    if opt.slice_npz is None:
        ...
        coords = torch.stack([yy, xx, torch.ones_like(xx) * opt.slice_var], dim=-1).reshape(-1,3).cuda()
        ...
        pv = np.hstack([pv, np.ones((pv.shape[0], 1)) * opt.slice_var])
        ...
        # save the sdf plane values 
        data = {'sdfslice': sdf_plane,
                'planev': pv,
                'planef': pf,
                'meshv': v,
                'meshf': f}
        outdir = os.path.dirname(opt.mesh_path)
        np.savez(os.path.join(outdir, 'sdf_plane.npz'), **data)
        print("Saved sdf slice visualization data to ", os.path.join(outdir, 'sdf_plane.npz'))
        exit()
    else:
        data = np.load(opt.slice_npz)
        pv = data['planev']
        pf = data['planef']
        v = data['meshv']
        f = data['meshf']
        sdf_plane = data['sdfslice']
        ps.init()
        ps.register_surface_mesh("mesh", v, f)
        plane = ps.register_surface_mesh("plane", pv, pf)
        plane.add_scalar_quantity("sdf", sdf_plane, enabled=True)
