...
def preprocess_image(images, from_range=(0, 1), cmap="gray"):
    min, max = from_range
    if (max - min) < 1e-6:
        images = (images - min) / ((max - min) + 1e-6)
    else:
...
def wandb_image(images, from_range=(0, 1)):
    ...
    image_grid = torchvision.utils.make_grid(torch.from_numpy(images).permute(2,0,1), nrow=1, pad_value=1) 
    image_grid = torchvision_F.to_pil_image(image_grid)
    wandb_image = wandb.Image(image_grid)
    return wandb_image

def write_sdf_summary(model, model_input, gt, model_output, writer, total_steps, prefix='train_', opt=None):
    ...
    with torch.no_grad():
        ...
        if total_steps in opt.steps_for_ckpt:
            np.save(f'{opt.outdir}/yz_slice_{total_steps}.npy', sdf_values)
        if opt.save_results_per_step > 0 \
        ...
        yz_fig_array = fig_to_array(fig)
        images.update({'yz_sdf_slice': wandb_image(yz_fig_array, from_range=(0,255))})
        ...
        sdf_values = dataio.lin2img(sdf_values).squeeze().cpu().numpy()
        if total_steps in opt.steps_for_ckpt:
            np.save(f'{opt.outdir}/xz_slice_{total_steps}.npy', sdf_values)
        if opt.save_results_per_step > 0 \
            and total_steps % opt.save_results_per_step == 0 \
            and total_steps <= opt.save_results_max_step:
            np.save(f'{opt.outdir}/xz_slice_{total_steps}.npy', sdf_values)
        ...
        xz_fig_array = fig_to_array(fig)
        images.update({'xz_sdf_slice': wandb_image(xz_fig_array, from_range=(0,255))})
        ...
        if total_steps in opt.steps_for_ckpt:
            np.save(f'{opt.outdir}/xy_slice_{total_steps}.npy', sdf_values)
        if opt.save_results_per_step > 0 \
            and total_steps % opt.save_results_per_step == 0 \
            and total_steps <= opt.save_results_max_step:
            np.save(f'{opt.outdir}/xy_slice_{total_steps}.npy', sdf_values)
        ...
        if 'stc' in opt.model_type and model.stochasticP and model.stc_type =='optim': 
            ...
            kernel_slice_2_raw = model.kernel_grid[:, :, idx:idx+1].repeat(1,1,3) #detach() #.cpu().numpy()
            v_min = torch.min(model.kernel_grid) #.cpu().numpy() # 1e-6
            v_max = torch.max(model.kernel_grid) #.cpu().numpy() # 0.06
            kernel_slice_0 = preprocess_image(kernel_slice_0_raw, from_range=(v_min, v_max))
            kernel_slice_1 = preprocess_image(kernel_slice_1_raw, from_range=(v_min, v_max))
            kernel_slice_2 = preprocess_image(kernel_slice_2_raw, from_range=(v_min, v_max))
            images.update({
                f"kernel_img_0": wandb.Image(Image.fromarray((255 * kernel_slice_0.detach().cpu().numpy()).astype(np.uint8))),
                f"kernel_img_1": wandb.Image(Image.fromarray((255 * kernel_slice_1.detach().cpu().numpy()).astype(np.uint8))),
                f"kernel_img_2": wandb.Image(Image.fromarray((255 * kernel_slice_2.detach().cpu().numpy()).astype(np.uint8)))
            })
            
            # log noise converted to level map [min_level-13, 13-15, 15-16,]
            kernel_slices = torch.cat([kernel_slice_0_raw[...,0:1],
                                       kernel_slice_1_raw[...,0:1],
                                       kernel_slice_2_raw[...,0:1]],dim=-1)
            
        
            vmin = torch.exp(torch.min(model.kernel_grid)).detach()
            vmax = torch.exp(torch.max(model.kernel_grid)).detach()
            if vmin == vmax:
                vmin = vmin - 1e-4
            
            kernel_slices_normalized = (torch.exp(kernel_slices) - vmin) / (vmax - vmin)
            a = model.num_levels - model.opt.optim_min_level
            r = model.opt.optim_r
            kernel_slices_levels = - (a**(1/r)*(1-kernel_slices_normalized) - a**(1/r))**r + model.num_levels
            kernel_slices_levels = kernel_slices_levels.clamp(0.0,16.0)
            
            kernel_slices_levels[kernel_slices_levels<=13] = 0
            kernel_slices_levels[torch.bitwise_and(kernel_slices_levels>13, kernel_slices_levels<=15)] = 0.5
            kernel_slices_levels[kernel_slices_levels>15] = 1.0
            images.update({
                f"kernel_level_0": wandb.Image(Image.fromarray((255 * kernel_slices_levels[...,0:1].repeat(1,1,3).detach().cpu().numpy()).astype(np.uint8))),
                f"kernel_level_1": wandb.Image(Image.fromarray((255 * kernel_slices_levels[...,1:2].repeat(1,1,3).detach().cpu().numpy()).astype(np.uint8))),
                f"kernel_level_2": wandb.Image(Image.fromarray((255 * kernel_slices_levels[...,2:3].repeat(1,1,3).detach().cpu().numpy()).astype(np.uint8)))
            })
            if opt.precompute_level_map == 'y':
                precomputed_level_slice_0 = opt.level_grid[idx:idx+1,:,:].permute(1,2,0).repeat(1,1,3)
                precomputed_level_slice_1 = opt.level_grid[:,idx:idx+1,:].permute(0,2,1).repeat(1,1,3)
                precomputed_level_slice_2 = opt.level_grid[:,:,idx:idx+1].repeat(1,1,3)
                images.update({
                    f"precomputed_level_0": wandb.Image(Image.fromarray((255 * precomputed_level_slice_0.detach().cpu().numpy()).astype(np.uint8))),
                    f"precomputed_level_1": wandb.Image(Image.fromarray((255 * precomputed_level_slice_1.detach().cpu().numpy()).astype(np.uint8))),
                    f"precomputed_level_2": wandb.Image(Image.fromarray((255 * precomputed_level_slice_2.detach().cpu().numpy()).astype(np.uint8)))
                }) 
