...
class PointCloud(Dataset):
    def __init__(self, pointcloud_path, on_surface_points, output_dir, keep_aspect_ratio=True, n_rings=2, precompute_level_map=False):
        super().__init__()
        ...
        if pointcloud_path.endswith('.xyz'): 
            ...
            # ps.init()
            # pc = ps.register_point_cloud("pointcloud", self.coords)
            # pc.add_vector_quantity("normals", self.normals, enabled=True)
            # ps.show()
            # exit()
        
        ### Construct a grid indicating a band around sampled points 
        if precompute_level_map:
            self.n_rings=n_rings
            data_dir = os.path.dirname(pointcloud_path)
            data_name = os.path.basename(pointcloud_path).split('.')[0]
            if os.path.exists(os.path.join(os.path.dirname(pointcloud_path),f'{data_name}_level_grid_{n_rings}rings.npy')):
                print("Loading level grid from", os.path.join(os.path.dirname(pointcloud_path),f'{data_name}_level_grid_{n_rings}rings.npy'))
                self.level_grid = np.load(os.path.join(os.path.dirname(pointcloud_path),f'{data_name}_level_grid_{n_rings}rings.npy'))
                self.level_grid = torch.from_numpy(self.level_grid).cuda()
            else:
                self.level_grid = torch.zeros(128,128,128)
                coords_integer = torch.from_numpy(((( self.coords + 1.0 ) / 2.0) * 127)).long() #[-1,1] to [0-1] to [0-128]
                coords_integer = torch.unique(coords_integer, dim=0)
                def set_neighbor(grid, coords):
                    x = coords[...,0]
                    y = coords[...,1]
                    z = coords[...,2]
                    for i in range(-1, 2):
                        for j in range(-1, 2):
                            for k in range(-1, 2):
                                coords_neighbor = torch.stack([x+i,y+j,z+k],dim=-1).clamp(0,127)
                                grid[coords_neighbor[:,0], coords_neighbor[:,1], coords_neighbor[:,2]] = 1 
                    return grid
                for i in range(n_rings):
                    self.level_grid = set_neighbor(self.level_grid, coords_integer)
                    coords_integer = torch.nonzero(self.level_grid == 1)
                #self.level_grid = self.level_grid.permute(0,2,1)
                #self.level_grid = self.level_grid.permute(1,0,2)
                #self.level_grid = self.level_grid.permute(1,2,0)
                self.level_grid = self.level_grid.permute(2,1,0)
                #self.level_grid = self.level_grid.permute(2,0,1)
                print(f"Saving level grid to {os.path.join(data_dir,f'{data_name}_level_grid_{n_rings}rings.npy')}")
                np.save(os.path.join(data_dir,f'{data_name}_level_grid_{n_rings}rings.npy'), self.level_grid.numpy())
                self.level_grid = self.level_grid.cuda()
